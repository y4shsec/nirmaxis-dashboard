...
export default function PatientProfile() {
  const { user } = useAuth();
  const [form, setForm] = useState({ name:"", email:"", address:"", area:"" });
  ...
  useEffect(() => {
    ...
    getPatient(user.uid).then(p => {
      if (p) setForm({ name: p.name||"", email: p.email||"", address: p.address||"", area: p.area||"" });
      setLoading(false);
    });
  }, [user]);
        ...
        <div style={{ maxWidth: 560, margin: "0 auto" }}>
          ...
          <div style={{
            ...
            <div style={{
              ...
              fontSize: 20, fontWeight: 700, color: "var(--teal-dark)", flexShrink: 0,
            }}>
            ...
            <form onSubmit={handleSave} style={{ padding: 24 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                <div>
                  ...
                  <select style={inputStyle} value={form.area} onChange={e => setForm(p => ({ ...p, area: e.target.value }))}>
                    <option value="">Select area</option>
                    {AREAS.map(a => <option key={a} value={a}>{a}</option>)}
                  </select>
                </div>
                <div>
                  <label style={{ display: "block", fontSize: 13, fontWeight: 500, marginBottom: 6 }}>Phone</label>
                  <input style={{ ...inputStyle, background: "#f8f8f8", color: "var(--text-light)" }}
                    type="text" value={user?.phoneNumber || ""} disabled />
                </div>
              </div>
